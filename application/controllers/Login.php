...
class Login extends CI_Controller {
	...
	public function cek_login(){
		...
		$cek_ip = $this->db->query("SELECT no_loket, ip_address FROM loket WHERE ip_address = '172.16.100.67';");
		...
		if ($query_cek->num_rows() > 0) {
			...
			$data_session = array(
				...
				'loket_temp' 	=> $data->loket_temp, 
				'company' 		=> $data->company_id);
			$this->session->set_userdata($data_session);
			//update status jadi on user dan temp_loket
			$update_status = $this->db->where(array('user_id' => $data->user_id))->update('user',array('status' => 'on')); 
			if (!$update_status) {
				echo '0';
			}
			echo '1';
