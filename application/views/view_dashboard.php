...
<div class="container-fluid">
	<div class="row">
		<div class="col-md-3"><br>
			<h3>Welcome,<br><strong><?= $this->session->userdata('nama'); ?> | LOKET #<?php echo $loket_temp;?></strong><br>
				<?php 
					if($this->session->userdata('company')=='2'){
						echo 'PT. Siprama Cakrawala';
					} else {
						echo 'PT. Impact Power Mandiri';
					}
		...
		<div class="col-md-3">
			...
			<div class="card-deck shadow-lg p-3 mb-5 bg-white rounded" id="print-antri">
			  <div class="card">
			      <div class="card-header text-center">LOKET SC #<?php echo $loket_temp;?> ANTRIAN SELANJUTNYA</div>
			  	<strong style="font-size: 80px; text-align: center;" id="no-waiting"><?= $data_waiting->nomor; ?></strong>
			    <div class="card-body">
			      <h5 class="card-title text-center">Total Antrian : <strong style="font-size: 20px; text-align: center;" id="status-waiting"><?= $data_waiting->total_waiting; ?></strong></h5>
			    </div>
			    <div class="card-footer bg-transparent text-center">Sedang Menunggu Panggilan</div>
			  </div>
			</div>		
			<br>
		...
		<div class="col-md-3">
			...
			<div class="card-deck shadow-lg p-3 mb-5 bg-white rounded" id="print-antri">
			  <div class="card">
			      <div class="card-header text-center">LOKET SC #<?php echo $loket_temp;?> | SEDANG INTERVIEW</div>
			  	<strong style="font-size: 80px; text-align: center;" id="no-servicing"><?= $data_servicing->nomor; ?></strong>
			    <div class="card-body">
			      <h5 class="card-title text-center">Total Sudah Interview: <strong style="font-size: 20px; text-align: center;" id="status-servicing"><?= $data_servicing->total_services; ?></strong></h5>
			    </div>
			    <div class="card-footer bg-transparent text-center" >Terima Kasih</div>
			  </div>
			</div>			
		</div>
		<div class="col-md-3 text-right">
<br>
			<a href="javascript:void(0)" class="btn btn-lg btn-danger" onclick="logout()"><h5>KELUAR / OFFLINE</h5></a>
		</div>
	</div>
</div>
		...
		$.ajax({
			...
			success:function(data){
				if (data==0) {
					...
					$("#status-waiting").html(data_waiting.total_waiting);
					$("#no-servicing").html(data_servicing.nomor);
					$("#status-servicing").html(data_servicing.total_services);
